...
// Desktop + tablet + phone: use Pointer Events instead of relying on a synthesized click.
// Mobile Safari/Chrome can cancel/delay click after touch/scroll/hover, which made the image planes
// feel unclickable on phones and tablets.
const raycaster = new THREE.Raycaster();
const pointer = new THREE.Vector2();

function selectPlaneAt(clientX, clientY) {
  const rect = renderer.domElement.getBoundingClientRect();

  // Convert the actual pointer/touch position to normalized device coordinates.
  ...
  if (intersects.length > 0) {
    ...
    activePlane = (activePlane === selected) ? null : selected;
  } else {
    activePlane = null;
  }
}
...
let tapStart = null;
const TAP_MOVE_LIMIT = 12; // px: lets normal scrolling pass without accidentally opening an image
...
function onPointerUp(e) {
  if (!tapStart) return;
  if (e.pointerId !== undefined && tapStart.id !== undefined && e.pointerId !== tapStart.id) return;

  const distance = Math.hypot(e.clientX - tapStart.x, e.clientY - tapStart.y);
  tapStart = null;

  // Treat it as a tap/click only when the finger/mouse did not drag.
  if (distance <= TAP_MOVE_LIMIT) {
    selectPlaneAt(e.clientX, e.clientY);
  }
}
...
if ('PointerEvent' in window) {
  // Capture phase makes this reliable even when touch is over transparent text/section layers.
  window.addEventListener('pointerdown', onPointerDown, true);
  window.addEventListener('pointerup', onPointerUp, true);
  window.addEventListener('pointercancel', cancelTap, true);
} else {
  // Fallback for very old iOS/browser versions.
  let touchStart = null;
  let lastTouchTime = 0;
  ...
  window.addEventListener('touchend', (e) => {
    ...
    const distance = Math.hypot(t.clientX - touchStart.x, t.clientY - touchStart.y);
    touchStart = null;
    if (distance <= TAP_MOVE_LIMIT) {
      lastTouchTime = Date.now();
      selectPlaneAt(t.clientX, t.clientY);
    }
  }, { passive: true, capture: true });
  ...
  window.addEventListener('click', (e) => {
    // Prevent the synthetic click after touch from toggling the same plane twice.
